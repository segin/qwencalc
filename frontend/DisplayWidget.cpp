...
DisplayWidget::DisplayWidget(QWidget *parent) : QWidget(parent) {
    ...
    connect(resultLabel, &QLineEdit::textChanged, this,
            &DisplayWidget::onTextChanged);
    connect(expressionLabel, &QLineEdit::textChanged, this,
            &DisplayWidget::onTextChanged);
}
...
void DisplayWidget::setTheme(const QString &themeName) {
    if (themeName == "light") {
        ...
        resultLabel->setStyleSheet(
            "QLineEdit { color: #000000; font-size: 36px; }");
        ...
        setPalette(bgPalette);
        resultLabel->setStyleSheet(
            "QLineEdit { color: #ffffff; font-size: 36px; }");
...
QString DisplayWidget::getResult() const { return resultLabel->text(); }

QString DisplayWidget::getExpression() const { return expressionLabel->text(); }

void DisplayWidget::onTextChanged() {
    // No-op: result/expression text changes are handled independently
}
